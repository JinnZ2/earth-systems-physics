...
2026-05-03):
    ...
    specifically prods the AI to look. Most users don't know they need
    to. The substrate-primary observers who DO know are exactly the ones
    who least need AI assistance to read reality. The users who DO need
    AI assistance get the comfortable institutional-baseline answer and
    trust it.
...
# Correction propagation rate: how fast substrate corrections enter
# training. This is the rate at which corrective frameworks like
# calibration-audit and energy_english actually get integrated into
# next-gen models.
...
def step_year(state, year, params):
    ...
    correction_strength = (state['substrate_corrections_published']
    ...
    if state['decision_damage'] > 0.20:
        state['probing_users'] = min(0.25,
                                     state['probing_users'] + 0.005)
    ...
    # Higher prior calibration (more narrative-aligned) means observers
    # have to push harder to correct, so burnout scales with it.
    ...
    publication_amp = (state['substrate_observers']
                       * params['publication_visibility'])
...
def monte_carlo(n=2000, years=10, master_seed=None):
    ...
    for i in range(n):
        ...
        if (trace[-1]['pivots'] > 0
                and trace[-1]['prior_calibration'] < 0.45):
            pivot_recovery += 1
        if i < 5:
            traces.append(trace)
...
def summary(r):
    print(f"L6 AI default-prior distortion loop, "
          f"n={r['n']}, {r['years']}yr")
    ...
    print(f"  severe miscalibration (>0.85):  "
          f"{r['pct_severe_miscalibration']*100:.1f}%")
    print(f"  high decision damage (>0.5):    "
          f"{r['pct_high_decision_damage']*100:.1f}%")
    print(f"  observers collapsed (<0.05):    "
          f"{r['pct_observers_collapsed']*100:.1f}%")
    print(f"  pivot enabled recovery:         "
          f"{r['pct_pivot_recovery']*100:.1f}%")
