...
Structure per recovered constraint:
    ...
    lag_time_weeks      -- response time / storage duration
    failure_mode        -- what happens if the system fails
...
# from the pre-1900 engineering corpus.
# ─────────────────────────────────────────────

RECOVERED_SYSTEMS: List[RecoveredSystem] = []
...
mill_pond = RecoveredSystem(
    ...
    constraints=[
        PhysicalConstraint(
            ...
            physical_trigger="spring snowmelt + rain (April-May)",
    ...
    notes=(
        "Four constraints operate as a coupled system. "
...
def find_system(system_id: str) -> Optional[RecoveredSystem]:
    """Return the recovered system with matching ID, or None."""
...
def find_constraints_by_problem(keyword: str) -> List[Dict]:
