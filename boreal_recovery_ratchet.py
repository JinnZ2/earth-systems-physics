...
# state tracked = RECOVERED (pre-cut) composition fraction x/K
# ─────────────────────────────────────────────────────────────

# conifer-forest recovery times from the meta-analysis
MODES = {
    "broadleaf_community": dict(tau=30.0,  K=1.0, r=0.15),
    "small_mammals":       dict(tau=55.0,  K=1.0, r=0.15),
    "vascular_plants":     dict(tau=85.0,  K=1.0, r=0.15),
    "lichens":             dict(tau=95.0,  K=1.0, r=0.10),
    "bryophytes":          dict(tau=120.0, K=1.0, r=0.10),  # >100, unrecovered in-window
}
...
def simulate(mode, T, n_rot,
    ...
    tau0, K, r = mode["tau"], mode["K"], mode["r"]
    L = 1.0
    x = K                                   # start mature
    traj = []
    for _ in range(n_rot):
        ...
        x = K - (K - x) * math.exp(-T / tau_eff)   # regrow to pre-cut value
        frac = x / K
