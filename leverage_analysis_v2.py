"""
leverage_analysis_v2.py
=======================

Find the highest-leverage intervention by computing elasticity
of deaths with respect to each parameter, ACROSS multiple
operating points (not just one saturated baseline).
...
Dependencies: stdlib + hormuz_cascade_audit (CascadeRun) only
"""
...
def deaths(point):
    """Run the cascade at a given operating point; return excess deaths."""
...
def apply(point, param, delta):
    """Apply an intervention delta to a parameter, clamped to physical bounds."""
    p = dict(point)
...
def build_leverage_matrix():
    """
    Returns (matrix, baselines):
      baselines[opname]            = deaths at baseline operating point
      matrix[opname][intervention_name] = lives saved by that intervention
    """
    baselines = {}
    matrix = {}
...
def rank_interventions(matrix, operating_point):
    """
    Rank interventions by lives saved at the given operating point,
    most to least. Returns the INTERVENTIONS list sorted in place-style
    (does not mutate the global).
    """
    return sorted(INTERVENTIONS,
                  key=lambda x: -matrix[operating_point][x[0]])
...
def main():
    ...
    # Ranked leverage at today's baseline
    print()
    print("=" * 78)
    ...
    for i, (iname, param, delta, itype) in enumerate(ranked, 1):
        ...
        pct = (100 * saved / baselines["today_q2_2026"]
               if baselines["today_q2_2026"] > 0 else 0)
...
THE GEOMETRY:
  ...
  Every additional month of disruption stacks against:
    ...
    - cumulative caloric deficit (mortality rate x time)
    - eroding distributional capacity (humanitarian fatigue)

  Mortality function: deaths ~ pop x base x (deficit^2) x time
                                                         ^
                                              LINEAR in duration
