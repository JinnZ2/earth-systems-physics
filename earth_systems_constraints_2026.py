...
# Add additional stresses and/or noise: collapse compresses 38-81%
# closer to present day.

COLLAPSE_COMPRESSION_MIN_PCT = 38
COLLAPSE_COMPRESSION_MAX_PCT = 81

# Implication: linear single-stressor projections systematically
# underestimate collapse proximity. Compound stressors are the rule,
# not the exception.
...
# Old assumption: meltwater discharge supplies primary iron to
# Southern Ocean, triggering algae blooms and CO2 drawdown ->
# negative (cooling) feedback.
# Field reality: meltwater iron contribution minimal; iron sourced
# from deep ocean water and resuspended sediments.

IRON_PRIMARY_SOURCE_OLD = "ice_meltwater_discharge"
IRON_PRIMARY_SOURCE_OBSERVED = "deep_ocean_water_and_sediments"

# Sediment core evidence (south of Antarctic Polar Front): iron supply
# HIGHEST during warm intervals — delivered by calving icebergs, NOT
# meltwater — and HIGH iron did NOT accelerate algae growth as expected.
...
def remove_iron_fertilization_carbon_sink(carbon_budget_dict):
    ...
    Returns (modified_budget, list_of_zeroed_keys).
    """
...
if __name__ == "__main__":
    print("EARTH SYSTEMS CONSTRAINT INTEGRATION 2026")
    print("=" * 60)
    print(f"\nGLACIER LAYER:")
    print(f"  2025 non-ice-sheet glacier loss: "
          f"{GLACIER_LOSS_2025_GT} +/- "
          f"{GLACIER_LOSS_2025_UNCERTAINTY_GT} Gt")
    print(f"  Greenland 2002-2025 avg:        "
          f"{GREENLAND_LOSS_2002_2025_AVG_GT_YR} Gt/yr")
    print(f"  Antarctica 2002-2025 avg:       "
          f"{ANTARCTICA_LOSS_2002_2025_AVG_GT_YR} Gt/yr")
    print(f"  Cumulative since 1975:          "
          f"{CUMULATIVE_GLACIER_LOSS_SINCE_1975_GT} Gt")

    print(f"\nECOSYSTEM COLLAPSE COMPRESSION:")
    print(f"  Compression range:              "
          f"{COLLAPSE_COMPRESSION_MIN_PCT}-"
          f"{COLLAPSE_COMPRESSION_MAX_PCT}%")
    print(f"  Coral tipping point crossed:    "
    ...
    print(f"\nIRON FERTILIZATION (West Antarctic):")
    print(f"  Hypothesis status:              "
          f"{IRON_FERTILIZATION_HYPOTHESIS_STATUS}")
    print(f"  Old source:                     "
          f"{IRON_PRIMARY_SOURCE_OLD}")
    print(f"  Observed source:                "
          f"{IRON_PRIMARY_SOURCE_OBSERVED}")
    print(f"  Assumed feedback sign:          "
          f"{ASSUMED_FEEDBACK_SIGN}")
    print(f"  Observed feedback sign:         "
          f"{OBSERVED_FEEDBACK_SIGN}")

    print(f"\nCONSTRAINT CHECK demos:")
    for assumption in (
        "iron_fertilization_carbon_sink",
        "linear_single_stressor_collapse_timeline",
        "stable_holocene_baseline",  # not in registry; should be CONDITIONAL
    ):
    ...
    for system, year in [
        ("tropical_ocean", 2026),
        ("tropical_forest", 2046),
        ("polar_ice_sheet", 2046),
        ("coral_reef", 2027),
        ("atmosphere_co2", 2026),
    ]:
        triggered, status = cascade_trigger_check(system, year)
        print(f"  {system}@{year}: triggered={triggered} ({status})")

    print(f"\nCOLLAPSE COMPRESSION demos:")
    print(f"  100yr baseline, 1 stressor:    "
          f"{apply_collapse_compression(100, 1)}")
    print(f"  100yr baseline, 3 stressors:   "
          f"{apply_collapse_compression(100, 3)}")

    print(f"\nIRON SINK REMOVAL demo:")
    budget = {
    ...
    print(f"  Original budget keys: {sorted(budget.keys())}")
    print(f"  Zeroed:               {zeroed}")
    print(f"  Resulting budget:     {new_budget}")
