...
#
# PATCHES applied:
#   magpause_standoff: Chapman-Ferraro formula in R_E units.
#     Correct: (2*B^2 / (mu0*P_dyn))^(1/6)  [already in R_E when B is surface field,
#     because the dipole-moment R_E^6 factors cancel]. Old formula divided by R_E
#     a second time and dropped the factor of 2 -> gave 1.4e-6 instead of ~10 R_E.
#   EEI: TOA imbalance compares F_in to OLR, not sigma*T_surface^4.
#     F_out now uses T_eff (effective emission temperature, ~255 K baseline),
#     which is the temperature the Earth radiates at from the top of atmosphere.
#     T_surface drives greenhouse forcing F_co2 only, not the OLR directly.
...
CLAIMS = {
    ...
    "T_eff_baseline": 255.0,  # effective emission temp (OLR baseline)        K
}
...
# ============================================================
# 4.  MAGNETOSPHERE
#     Chapman-Ferraro pressure balance:
#       R_mp/R_E = (2 * B_surface^2 / (mu0 * rho_sw * v_sw^2))^(1/6)
#     The factor of 2 comes from the magnetic pressure doubling at the
#     stagnation point; the R_E^6 from the dipole moment (M = B*R_E^3 / mu0 * 4pi)
#     cancels to give the result directly in R_E when using surface B.
# ============================================================
def layer_magnetosphere():
    ...
    couplings = [
        FuncCoupling("P_dyn", ["sw_density","sw_velocity"], "P_dyn",
                     lambda p: p["sw_density"]*MP * p["sw_velocity"]**2),
        FuncCoupling("standoff", ["B_surface","P_dyn"], "magpause_standoff",
                     lambda p: (2.0 * p["B_surface"]**2
                                / (MU0 * max(p["P_dyn"], 1e-30)))**(1/6)
    ...
    return LayerSpec(axes, couplings, constraints, "pressure balance -> standoff in R_E")
...
# 6.  ATMOSPHERE
#     TOA energy balance:
#       F_in  = S0*(1-albedo)/4                  [absorbed solar]
#       F_out = sigma * T_eff^4                  [OLR at effective emission temp]
#       F_co2 = 5.35 * ln(CO2/280)              [radiative forcing]
#       EEI   = (F_in + F_co2) - F_out          [imbalance; observed ~0.9 W/m^2 now]
#
#     T_eff is the effective emission temperature (~255 K baseline), NOT T_surface.
#     T_surface (~288 K) determines the greenhouse effect (F_co2 acts as a delta
#     on T_eff implicitly); using T_surface in F_out would compute surface emission
#     to space, which is not what happens — the atmosphere intercepts it.
#
#     convective gate: env_lapse > dry adiabatic -> absolute instability
# ============================================================
def layer_atmosphere():
    axes = ["albedo", "T_surface", "T_eff", "co2_ppm", "env_lapse",
            "F_in", "F_out", "F_co2", "EEI"]
    couplings = [
        FuncCoupling("F_in",  ["albedo"], "F_in",
                     lambda p: S0*(1 - p["albedo"])/4),
        FuncCoupling("F_out", ["T_eff"], "F_out",
                     lambda p: SIGMA * p["T_eff"]**4),
        FuncCoupling("F_co2", ["co2_ppm"], "F_co2",
                     lambda p: 5.35*math.log(max(p["co2_ppm"], 1.0)/280.0)),
        FuncCoupling("EEI",   ["F_in","F_out","F_co2"], "EEI",
                     lambda p: (p["F_in"] + p["F_co2"]) - p["F_out"]),
    ]
    constraints = [
        Constraint("radiative_balance", {"EEI":1.0}, CLAIMS["EEI_max"], "<",
                   "|TOA imbalance| > 1.5 W/m^2 -> forced disequilibrium"),
...
# ============================================================
# 8.  LITHOSPHERE
#     GIA strain + Mohr-Coulomb failure gate
# ============================================================
...
# ============================================================
# 9.  CORE  (v5 oscillation, bottom of the cascade)
# ============================================================
def layer_core():
...
# ============================================================
# 10. BIOSPHERE
#     NPP - Q10 respiration; biosphere flips source when net_c_uptake < 0
# ============================================================
def layer_biosphere():
...
# ============================================================
# 12. DEMO  -- holocene / forced / hosing
# ============================================================
def demo():
    ...
    base_state = {
        ...
        "albedo": 0.30, "T_surface": 288.0, "T_eff": 255.0, "env_lapse": 6.5,
    ...
    def run(label, co2, melt, T_eff):
        coords = dict(base_state)
        coords["co2_ppm"] = co2
        coords["ice_melt_rate"] = melt
        coords["T_eff"] = T_eff
        ...
        print(f"\n[{label}]  CO2={co2}  melt={melt}  T_eff={T_eff}K")
        for k in ["EEI","F_in","F_out","amoc_strength","emergent_mantle_anomaly",
    ...
    # holocene: CO2=280, minimal melt, T_eff~255 K (near balance)
    run("holocene", 280,  0.01, 255.0)
    # forced: CO2=420, current melt, T_eff shifts ~1K via CO2 forcing
    run("forced",   420,  0.04, 256.0)
    # hosing: CO2=500, large melt, AMOC risk
    run("hosing",   500,  0.12, 257.0)
