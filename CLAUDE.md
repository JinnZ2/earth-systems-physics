...
pytest -v                             # Run test suite (1309 tests)
...
├── risk_posture.py                   # Same PHC 2026 data read two ways: PUBLISHED vs MITIGATION under 6 declared rules + R9 latency dating
├── planetary_health_check_2026.py     # PHC 2026 (PBScience/PIK) planetary-boundary CV table; 7 of 9; CV_MISMATCH map for legacy CVs
├── global_temperature_august_2026.py  # Observed Aug 2026: warmest August (ERA5/NOAA/NASA), ERA5 joint warmest month; the ENSO half of the 2025 superposition
├── archive_siting_bias.py             # MARKER: preservation-set archive location vs source location; a-priori bias sign + operator-class correctability
├── ARCHIVE_SITING_BIAS.md             # Companion prose marker (SHAPE, calibration points, claim table, handoff)
├── test_smoke.py                      # 1136 tests — all layers, scenarios, validators, audits, observed-state modules
...
│   └── catalogs/                      # 41 .jsonl catalogs auto-exported from sources
│
├── experiments/
...
Framework: **pytest** — 1309 tests covering all layers, scenarios, validators, magnomechanical integration, climate-scheme audits, systems audits, epistemology models, sensor-corruption models, and consequence dynamics.
Framework: **pytest** — 1308 tests covering all layers, scenarios, validators, magnomechanical integration, climate-scheme audits, systems audits, epistemology models, sensor-corruption models, and consequence dynamics.
...
| `global_temperature_august_2026.py` | **Observed state, August 2026** — the warmest August in ERA5, NOAAGlobalTemp and GISTEMP, and ERA5's joint warmest month on record (C3S, NOAA NCEI, NSIDC, NOAA CPC, Berkeley Earth bulletins, published September 2026). ERA5: 16.96 °C absolute, +0.85 °C vs 1991-2020, **+1.65 °C vs 1850-1900**; ties July 2023 in absolute terms (< 0.01 °C) but `absolute_vs_anomaly_tie` shows the anomaly is 0.13 °C LARGER because August's climatology is 0.12 °C cooler than July's. NOAA: +1.32 °C vs 20th century, +0.08 °C over the 2023/2024 tie, 13.2% of the surface record-warm; `record_margins` shows the ERA5 margin (0.14) is 1.75× NOAA's — rank is robust across datasets, margin is not. 12-month mean 1.48 °C, down from 1.64 °C (Sep 2023-Aug 2024): `running_mean_modulation` separates the ~0.04 °C of trend from a ~0.20 °C ENSO swing; `paris_threshold_status` shows no crossing on the 20-year window. `enso_superposition_check` closes the loop with `state_of_the_climate_2025.enso_decoupling_check`: a neutral 2025 ranked top-three, a strong El Niño August (weekly Niño 3.4 **+2.7 °C**, CPC >90% very strong through winter 2026-27) sits 0.17 °C above the running mean — a realised sensitivity floor of 0.063 °C per °C Niño 3.4. Attribute the LEVEL to the baseline and the EXCURSION to ENSO. Extra-polar SST 21.07 °C (August record) with the all-time daily record 21.10 °C set on 22 August, off the March-April seasonal peak. NSIDC: Antarctic 16.46 Mkm² (3rd lowest), Arctic 5.56 Mkm² (7th lowest). `el_nino_following_year_pattern` records 1998/2016/2024 as a pattern, explicitly not a projection. `PROVENANCE` per constant with comparison points from earlier bulletins marked; `baseline_overrides()` hands `SST_enso` = 2.7 to the cascade engine, where the `super_el_nino_2026_27` scenario drives Layer 4's Bjerknes feedback. Stdlib only. CC0. |
| `planetary_health_check_2026.py` | **Observed planetary-boundary state** — Planetary Health Check 2026, Summary Report, PBScience/PIK (released 2026-09-21; some data predates 2026). `CONTROL_VARIABLES` holds the 2026 CVs with boundary and high-risk line: CO2 426 ppm / RF +3.10 W/m², E/MSY 100-1000 / HANPP 21-30%, forest 64% of potential, blue 22.6% / green 22.0% of ice-free land, P 18.2 / N 165 Tg/yr, Ω_arag 2.85 vs 2.86, ΔAOD 0.07 (S. Asia AOD 0.32). Ozone: CV extra-polar 60N-60S 11-yr mean, value NOT_IN_TEXT (chart-only, never read off the chart), boundary 277.4 DU = 292 − 5% (PRELIMINARY), zone SAFE; legacy `ozone_DU` slot flagged CV_MISMATCH. Novel entities: NO_QUANTIFIED_CV, TRANSGRESSED_BY_PROXY; `novel_entities_status` accepts bool/enum and raises TypeError on numbers. `reproduce_check()` shows the zone logic reproduces the report: **7 of 9** transgressed. **Measurand rule:** `CV_MISMATCHES` maps legacy CVs (ocean pH, freshwater km³, cropland fraction, global AOD, P ocean-flow) to their 2026 replacements; `layer_6_biosphere.planetary_boundary_status()` takes the 2026 CVs as keywords, keeps the legacy slots as superseded fields, and counts N+P once as biogeochemical flows. `ocean_acidification_status` returns BREACHED_WITHIN_DATASET_SPREAD for a 0.01 Ω margin rather than a hard binary; `aerosol_status` returns GLOBAL_ONLY_SCOPE without the regional field. `LAND_SINK_NOTE`: ESMs likely overestimate land-sink resilience — carried into NEP, noted not corrected. Stdlib only. CC0. |
| `thwaites_teis_2026.py` | **Thwaites Eastern Ice Shelf, September 2026 imagery read (Kavik)** — OBSERVED_QUALITATIVE, not a measurement. O1 cracks lengthening, O2 crack count rising, O3 more dark spots, O4 thinning adjacent to existing cracks. O3 reading is sensor-dependent (sensor not recorded) with confounds carried: optical low-sun shadowing in crevasse fields makes O3 non-independent of O2; in SAR, September melt is unlikely and wind-roughened open water can image bright. O4 matched to Wild et al. 2024 (J. Glaciology, doi:10.1017/jog.2024.64; Sentinel-1 2014 to mid-2023). `LITERATURE` grades every source CITED_SECONDARY (located by search; full text not opened) and records corrections to the supplied Sept 2026 paper list (Williams et al. not 'Bradley et al.'; Pham is 2025; Goldberg et al. is a preprint and its 2.6 mm/yr figure is unlocated). Coupling O4<->O1/O2 is a DERIVED positive loop running before the spring rift window, with a falsifier: loop predicts thinning localized at crack edges, a common driver predicts distributed thinning. Published analogues of the loop: Benn 2022 (damage↔strain), Banerjee 2025 (fracturing↔acceleration); common-driver candidates carried with their status. MITIGATION reading under two newly declared rules — R7 precursors active → breakup UNDERWAY (detachment not claimed observed; PUBLISHED carries the source wording, Pettit et al. 2021 AGU abstract 'may be initiated as soon as 2026', not the read's 'very likely'), R8 pre-acceleration published rate → FLOOR (Wild 2024: 2.85 m/d early 2023, ~1041 m/yr). `quantified_cv()` upgrades only with rift length, crack count, dark-area fraction AND sensor, scene dates, AOI, detection threshold. Stdlib only. CC0. |
| `risk_posture.py` | **Same PHC 2026 data, two declared readings.** PUBLISHED = value and zone as the source states them; MITIGATION = the same values resolved by six declared, reversible rules that favour preparing over waiting: R1 adverse end of a range, R2 thin margin inside dataset spread → crossed, R3 unmeasured never safe, R4 regional value governs in-region, R5 source-stated model bias applied by direction only, R6 preliminary/proposed/provisional flags carried (SAFE → SAFE_PRELIMINARY). Moves no boundary. R2's width (`R2_SPREAD_FRACTION` = 0.02 of the boundary) is UNSOURCED (PHC publishes no spread) and printed with every hit. Carries its own copy of the table; `TestRiskPosture` fails if it drifts from `planetary_health_check_2026`. Stdlib only. CC0. |
| `thwaites_teis_2026.py` | **Thwaites Eastern Ice Shelf, September 2026 imagery read (Kavik)** — OBSERVED_QUALITATIVE, not a measurement. O1 cracks lengthening, O2 crack count rising, O3 more dark spots, O4 thinning adjacent to existing cracks. O3 reading is sensor-dependent (sensor not recorded) with confounds carried: optical low-sun shadowing in crevasse fields makes O3 non-independent of O2; in SAR, September melt is unlikely and wind-roughened open water can image bright. O4 matched to a J. Glaciology Sentinel-1 2014-2024 pattern (citation incomplete, UNCITED). Coupling O4<->O1/O2 is a DERIVED positive loop running before the spring rift window, with a falsifier: loop predicts thinning localized at crack edges, a common driver predicts distributed thinning. MITIGATION reading under two newly declared rules — R7 precursors active → breakup UNDERWAY (detachment not claimed observed), R8 pre-acceleration published rate → FLOOR; the published forecast and flow values are UNCITED / NOT_IN_TEXT. `quantified_cv()` upgrades only with rift length, crack count, dark-area fraction AND sensor, scene dates, AOI, detection threshold. Stdlib only. CC0. |
| `risk_posture.py` | **Same PHC 2026 data, two declared readings.** PUBLISHED = value and zone as the source states them; MITIGATION = the same values resolved by six declared, reversible rules that favour preparing over waiting: R1 adverse end of a range, R2 thin margin inside dataset spread → crossed, R3 unmeasured never safe, R4 regional value governs in-region, R5 source-stated model bias applied by direction only, R6 preliminary/proposed/provisional flags carried (SAFE → SAFE_PRELIMINARY). Moves no boundary. R2's width (`R2_SPREAD_FRACTION` = 0.02 of the boundary) is UNSOURCED (PHC publishes no spread) and printed with every hit. R9_LATENCY (marked 2026-09-23): when a published data window ends before the latest observation, the observation governs CURRENT state and the published figure becomes the PRIOR; `LATENCY` records window / publication / latest-observation dates for IMBIE, Thwaites glacial quakes, Thwaites eastern-shelf flow and the ESA Thwaites image -- dates only, no value carried forward. Numbered R9 because `thwaites_teis_2026.py` already declares R7/R8 on the same rule family; `test_rule_ids_unique_across_family` guards the numbering. Carries its own copy of the table; `TestRiskPosture` fails if it drifts from `planetary_health_check_2026`. Stdlib only. CC0. |
...
| `archive_siting_bias.py` | **MARKER** (companion prose in `ARCHIVE_SITING_BIAS.md`) for a sensed shape: a proxy archive is preserved where preservation conditions hold (cold/dry/anoxic/reef/limiting-season), which is NOT where the signal originates. Where the two anti-correlate, remote reconstructions carry a bias whose **sign is knowable a priori**. Attenuation factor `A = remote_estimate / at_source_estimate`; `bias_direction`. Calibration 1 (physical transport): Lamantia et al. 2026 (*Nature*, doi 10.1038/s41586-026-10938-1) CH4 four-box inversion — `ch4_attenuation()` returns A_TS=0.65, A_tropics=0.77; `ch4_error_localization()` shows 88% of the correction lands in the one unmeasured box; `siting_bias_vs_envelope()` shows the missing-box term (50 Tg/yr) exceeds the whole quantified parameter envelope (17.5). An independent `FourBoxModel` (steady-state loss+exchange, NOT the Zenodo code) reproduces A<1 on a self-consistent synthetic truth and, via `soft_prior_sweep`, exposes that prescribing one tropical box while interpolating its neighbour is itself an opposite-sign bias. Calibration 2 (statistical, non-stationary): ENSO teleconnection reconstructions — the CONTRAST, where the bias sign is frequency-dependent and correctability drops to boundable-only. `classify_operator()` is the decision procedure: physical+stationary → correctable a priori, statistical/routing-rule+non-stationary → boundable only, with a measurable `stationarity_index` vs the model's own perturbation envelope. Tests T-1..T-4 (`lifetime_scaling`, `enso_band_attenuation`, `a_priori_bound`, `network_geometry`) each close a claim. Nine-row `CLAIM_TABLE` (ASB-01..09) with refuter per claim; `SIBLING_ARCHIVES` lists seven archive classes predicted to carry the same bias. Refutation protocol: update the claim, never retune. Stdlib only. CC0. |
...
├── composition_recipes.md  Cross-module analysis patterns
├── index.json              Provenance + schema for every catalog
└── catalogs/               41 .jsonl catalogs (316 records total)
    ...
    ├── global_temperature_august_2026_provenance.jsonl (16) # global_temperature_august_2026
    ├── archive_siting_bias_claims.jsonl          (9)  # archive_siting_bias
    ├── archive_siting_bias_siblings.jsonl        (7)  # archive_siting_bias
    ├── planetary_health_check_2026_cvs.jsonl     (12) # planetary_health_check_2026
    └── planetary_health_check_2026_unquantified.jsonl (2) # planetary_health_check_2026
```
...
**SCENARIOS** (in `cascade_engine.py`): 23 pre-configured forcing functions — CO2 pulse, AMOC collapse, geomagnetic storm, solar proton event, Morin transition, BIF magnonic crystal, ocean timber dumping, CDW intrusion acceleration, anoxia expansion, the observed 2026-27 super El Niño (`super_el_nino_2026_27`, +2.7 K on `SST_enso`), and others. The ocean-timber scenario pairs with `run_ocean_timber_full_audit()`, which runs the cascade AND the multi-layer thermodynamic audit in `ocean_timber_sequestration_audit.py` together.
